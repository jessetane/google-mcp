...
function init () {
	...
	if (!columns.includes('readonly')) sqlite.exec('ALTER TABLE oauth_states ADD COLUMN readonly INTEGER NOT NULL DEFAULT 0;')
}
...
function create (opts = {}) {
	...
	const now = Date.now()
	sqlite.prepare(`
		INSERT INTO oauth_states (state, client_redirect_uri, client_state, code_challenge, code_challenge_method, readonly, ip, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`).run(state, clientRedirectUri, clientState, codeChallenge, codeChallengeMethod, readonly ? 1 : 0, ip, now)
	return state
}
